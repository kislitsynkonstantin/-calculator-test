...
СНИМОК = {
    ...
    "snapshot": {
        ...
        # Договор и заказчик не заполнены нарочно: на этих полях и появлялись
        # голые линейки без названий.
        "технология": "Каркас", "утепление": "150 мм",
...
def главная():
    ...
    with sync_playwright() as pw:
        ...
        for окно, ш, в in ОКНА:
            ...
            д = стр.evaluate("""() => {
              const реквизиты = [...document.querySelectorAll('.bl-req > div')].map(э => ({
                название: (э.querySelector('span:not(.bl-fill)') || {}).textContent || '',
                значение: (э.querySelector('b') || {}).textContent || '',
                место: !!э.querySelector('.bl-fill'),
                линейка: parseFloat(getComputedStyle(э).borderBottomWidth) || 0,
                пусто: !э.children.length,
              }));
            ...
            # Полоска без названия — вот что видит человек: линейка нарисована,
            # а что это за поле, сказать нечем. Незаполненное поле обязано
            # оставить название и место под него, как на листе менеджера.
            for р in д["реквизиты"]:
                # Клетка без единого потомка — поле, у которого пропало
                # название. Линейку у неё снимает CSS, поэтому по одной
                # линейке это не поймать: смотрим на саму пустоту.
                if р["пусто"]:
                    НАХОДКИ.append(f"[{окно}] в реквизитах пустая клетка — "
                                   "у поля пропало название")
                elif р["линейка"] and not р["название"]:
                    плохо = (f"[{окно}] в реквизитах линейка без названия: "
                             f"значение={р['значение']!r}, пусто={р['пусто']}")
                    НАХОДКИ.append(плохо)
                elif р["название"] and not р["значение"] and not р["место"]:
                    НАХОДКИ.append(f"[{окно}] поле «{р['название']}» не заполнено, "
                                   "но и места под него нет")
            if not any(р["место"] for р in д["реквизиты"]):
                НАХОДКИ.append(f"[{окно}] ни одного незаполненного поля в снимке — "
                               "проба проверяет не тот случай")
            if д["хвост"] != "bl-foot":
