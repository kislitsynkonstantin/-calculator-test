...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            # ── новый расчёт: код уходит в журнал из самого сохранения ──
            # Константин 23.09.2026, снимком журнала: «менеджер сохраняет новый
            # расчёт, но код пресета не появляется в журнале действий». Код
            # искали задним числом, по открытому пресету в памяти браузера;
            # теперь он берётся там, где выдан. Вторая половина проверки —
            # память браузера нарочно теряет номер: событие обязано унести код
            # всё равно.
            н = стр.evaluate("""async () => {
              window.__ТАБЛИЦЫ.events = [];
              _sbUser = { id: 'u-проба' };
              saveAllPresets({});
              setActivePreset(null);
              // Диалог имени подтверждаем сразу — как человек, нажавший
              // «Сохранить».
              showPresetNameDialog = (имя, кб) => кб(имя || 'Проба расчёт');
              await savePreset();
              await new Promise(r => setTimeout(r, 700));
              const обычное = (window.__ТАБЛИЦЫ.events.slice(-1)[0] || {}).details;

              // Теперь память браузера теряет номер: сохраняем пресеты без него.
              const прежнее = saveAllPresets;
              saveAllPresets = (п) => {
                const без = JSON.parse(JSON.stringify(п || {}));
                Object.values(без).forEach(о => { delete о.shortCode; delete о.sharedId; });
                return прежнее(без);
              };
              window.__ТАБЛИЦЫ.events = [];
              setActivePreset(null);
              await savePreset();
              await new Promise(r => setTimeout(r, 700));
              const безПамяти = (window.__ТАБЛИЦЫ.events.slice(-1)[0] || {}).details;
              saveAllPresets = прежнее;
              return { обычное, безПамяти };
            }""")
            for имя, д in (("обычное сохранение", (н or {}).get("обычное")),
                           ("память браузера потеряла номер", (н or {}).get("безПамяти"))):
                код = (д or {}).get("presetCode")
                if not код or not str(код).isdigit():
                    плохо(f"[{имя}] в событии «Расчёт сохранён в пресет» кода нет "
                          f"({д}) — в журнале строку не по чему открыть")
