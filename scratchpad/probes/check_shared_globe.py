...
Проба держит на 390 и 1440, в «Бланке» и «Модерне»:
  • на «Общих» у опубликованного для всех — глобус в строке кода перед «Код»,
    метки «Для всех» нет; у своего общего метки «Мой» нет — свой отмечен
    глобусом цветом схемы, чужой глобус серый (Константин, 26.09.2026:
    «глобус в общих и в моих делай контуры цветом темы оформления. А «Мой»
    убери»);
  ...
  • на «Моих» глобус опубликованного того же цвета схемы, что свой на «Общих»;
...
СЦЕНАРИЙ = """async () => {
  ...
  const мои = [...document.querySelectorAll('#presetList .pcard')].map(к => ({ ид: к.dataset.pid,
    ...
    цвет: (() => { const b = к.querySelector('.pcard-code-row .pst'); return b ? getComputedStyle(b).color : ''; })(),
  ...
  const общие = [...document.querySelectorAll('#sharedPresetList .shared-pcard')].map(к => {
    ...
    return { код: к.dataset.scode, метка: (к.querySelector('.shared-pcard-badge') || {}).textContent || '',
      цвет: b ? getComputedStyle(b).color : '',
  ...
  return { мои, общие, есть_мой: !!document.querySelector('#sharedPresetList .badge-pub-mine') };
}"""
...
def главная():
        ...
        with sync_playwright() as pw:
            ...
            for ш in (390, 1440):
                ...
                for тема in ("blank", "light"):
                    ...
                    if р.get("есть_мой"): плохо(н + ": на «Общих» осталась метка «Мой»")
                    ...
                    свой, чужой = общ.get("333333", {}).get("цвет"), общ.get("775165", {}).get("цвет")
                    if not свой or свой == чужой: плохо(н + f": свой глобус на «Общих» того же цвета, что чужой ({свой})")
                    if мои.get("m2", {}).get("цвет") != свой: плохо(н + f": глобус на «Моих» ({мои.get('m2', {}).get('цвет')}) не того цвета, что свой на «Общих» ({свой})")
                    if свой and свой != чужой and мои.get("m1", {}).get("цвет") == свой: плохо(н + ": облако неопубликованного окрашено цветом схемы")
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:30]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: на «Общих» глобус перед «Код» вместо метки «Для всех», свой глобус цветом схемы на обеих вкладках, «Мой» нет, ветки «только администраторам» нет; "
          "на «Моих» меток нет, опубликованный — с глобусом; значки одного размера и не прилипли — на 390 и 1440, в обеих темах.")
