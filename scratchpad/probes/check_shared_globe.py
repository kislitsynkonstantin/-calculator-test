...
Проба держит на 390 и 1440, в «Бланке» и «Модерне»:
  ...
  • у пресета «только администраторам» — щит и метка «Admin only»;
...
def главная():
        ...
        with sync_playwright() as pw:
            ...
            for ш in (390, 1440):
                ...
                for тема in ("blank", "light"):
                    ...
                    if not общ.get("444444", {}).get("щит") or "Admin" not in общ.get("444444", {}).get("метка", ""):
                        плохо(н + ": у пресета «только администраторам» нет щита или метки")
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:30]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: на «Общих» глобус перед «Код» вместо метки «Для всех», у своего общего — «Мой», у админского — щит; "
