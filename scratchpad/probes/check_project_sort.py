...
Проба меряет поведение и место, а не разметку:
  ...
  • выбор уходит в настройки аккаунта — в строку профиля, а не только в
    память браузера;
  • работа с проектом в этом заходе поднимает его в частоте сразу, и ровно
    на один день, сколько бы событий в нём ни было;
...
ТАБЛИЦЫ = dict(ДАННЫЕ,
               ...
               events=[])
ЧАСТОТА = [
    {"project_name": ВИГО, "days": 7, "last_used": "2026-09-10T10:00:00Z"},
    {"project_name": БЕРЛИН, "days": 3, "last_used": "2026-09-20T10:00:00Z"},
    {"project_name": ЭССЕН, "days": 3, "last_used": "2026-09-01T10:00:00Z"},
]
ПОДГОТОВКА_JS = ("window.__ТАБЛИЦЫ = Object.assign(window.__ТАБЛИЦЫ || {}, "
                 ...
                 "window.__RPC.project_usage = " + json.dumps(ЧАСТОТА, ensure_ascii=False) + ";"
...
def проверить(стр, где):
    ...
    # ── работа с проектом: сразу и ровно на день ──
    нажать(стр, "freq")
    ...
    if было is None or стало is None:
        ...
        плохо(f"[{где}] работа с проектом в этом заходе засчитана {стало - было} днями, ждали 1")
...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            стр.add_init_script(ЗАГЛУШКА)
            стр.add_init_script(ПОДГОТОВКА_JS)
            стр.add_init_script("window.__RPC.project_usage = () => { throw new Error('нет функции'); };")
            стр.goto(f"http://127.0.0.1:{порт}/index.html", wait_until="load")
            стр.wait_for_timeout(2500)
    ...
    print("Чисто: три значка выстраивают список своим порядком и не закрывают его, "
          "выбор уходит в аккаунт, работа с проектом засчитывается одним днём сразу, "
