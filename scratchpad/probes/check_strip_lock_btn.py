...
«Модерне», днём и ночью:
  • на 390 замок во всю высоту двух строк: зазоры сверху и снизу до края
    полоски равны (±2 px) и не больше 10 px, кнопка не ниже 32 px и не уже 40;
  • поле нажатия по вертикали не меньше 44 px;
...
def главная():
                ...
                for тема in ("blank", "light"):
                    for ночь in (False, True):
                        ...
                        if ш == 390:
                            ...
                            if max(р["сверху"], р["снизу"]) > 10: плохо(н + f": замок не во всю высоту полоски ({р['сверху']} / {р['снизу']})")
                            if р["кнопка"][3] < 32 or р["кнопка"][2] < 40: плохо(н + f": замок мал ({р['кнопка'][2]}×{р['кнопка'][3]})")
                        else:
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:40]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: на телефоне замок автора во всю высоту полоски и по центру, поле нажатия не меньше 44 px, "
