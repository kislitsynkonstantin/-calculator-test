...
Проверяется:
  ...
  • под последней строкой на телефоне остаётся пустое место: раздел стоит
    в окне последним, и адресная строка браузера накрывала его — докрутить
...
ПУНКТОВ = 14
НАЖАТИЕ = 44
ХВОСТ = 60      # пустое место под последней строкой на телефоне
...
def проверить(бр, порт, тема, ширина):
    ...
    # ── Хвост под последней строкой ──
    # Раздел «Сбросить всё» стоит в окне последним, и на телефоне адресная
    # строка браузера накрывала его: докрутить до конца можно, прочитать
    # нельзя. Браузер без окружения этой полосы не рисует, поэтому меряем то,
    # что от неё спасает, — пустое место под последней строкой при полной
    # прокрутке.
    if ширина <= 480:
        хвост = спросить(стр, тема, """async () => {
          const короб = document.getElementById('settingsBox');
          короб.scrollTop = короб.scrollHeight;
          await new Promise(r => setTimeout(r, 500));
          const ш = document.querySelector('#settingsBody .st-fold');
          return Math.round(короб.getBoundingClientRect().bottom
                            - ш.getBoundingClientRect().bottom);
        }""")
        if хвост is not None and хвост < ХВОСТ:
            плохо(тема, f"под последней строкой всего {хвост} px при мерке {ХВОСТ} — "
                        "на телефоне её закроет адресная строка браузера")
