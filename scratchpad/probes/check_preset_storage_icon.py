...
«по размеру как компьютер». И в тот же день: «менеджеру без разницы, где
хранится пресет. Это мне важно» — поэтому значок только у администратора.
Про «Активный»: «поставь серую точку, как возле активного пресета и на
кнопке пресетов. Сейчас кнопка солнца вообще не к месту».

Проба держит:
  • у администратора в правом нижнем углу каждой своей карточки значок:
    глобус — пресет в базе, экран — ждёт связи в очереди браузера;
  • у менеджера значка нет;
  • связь вернулась, очередь ушла — экран сам сменяется глобусом;
  ...
  • на 390 / 768 / 1440, в «Бланке» и «Модерне», днём и ночью значок не
    ...
    в пустоте; глобус не меньше экрана;
...
МЕРА = """() => [...document.querySelectorAll('#presetPanel .pcard')].map(card => {
  ...
  const г = b.querySelector('svg').querySelector('circle, rect').getBBox();
  let сосед = 1e9, кто = '';
...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            for ш in (390, 768, 1440):
                ...
                for тема in ("blank", "light"):
                    for ночь in (False, True):
                        ...
                        if м["p_base"].get("нет") or м["p_loc"].get("нет"):
                            плохо(метка + ": у администратора нет значка")
                            continue
                        if м["p_base"]["лок"] or not м["p_loc"]["лок"]:
                            плохо(метка + ": значок не отвечает месту хранения")
                        if м["p_base"]["знак"] < м["p_loc"]["знак"] * 0.95:
                            плохо(метка + f": глобус мельче экрана ({м['p_base']['знак']:.0f} против {м['p_loc']['знак']:.0f})")
                        for х in м.values():
                            н = f"{метка} {х['ид']}"
                ...
                # Связь вернулась — экран сменился глобусом.
                ...
                # Менеджер значка не видит.
                стр.evaluate(РАЗЛОЖИТЬ, "manager")
                if стр.evaluate("() => document.querySelectorAll('#presetPanel .pst').length"):
                    плохо(f"{ш}: у менеджера виден значок места хранения")
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:40]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: у администратора в правом нижнем углу карточки глобус — пресет в базе — или экран — ждёт связи; "
          "очередь ушла — значок сменился сам; у менеджера значка нет; на 390/768/1440 в обеих темах днём и ночью "
