...
Проба держит:
  • в строке кода, слева от «Код», на каждой своей карточке значок — и у менеджера, и у
...
МЕРА = """() => [...document.querySelectorAll('#presetPanel .pcard')].map(card => {
  ...
  return { ид: card.dataset.pid, лок: b.classList.contains('loc'), знак: г.width * г.height,
           ...
           доКрая: Math.round(r.left - c.left), кодПравее: (() => { const л = card.querySelector('.pcard-code-label'); return л ? л.getBoundingClientRect().left >= r.right : false; })(), вСтрокеКода: !!b.closest('.pcard-code-row'), доНиза: Math.round(c.bottom - r.bottom), сосед: Math.round(сосед), кто,
...
def главная():
            ...
            for ш in (390, 768, 1440):
                ...
                for тема in ("blank", "light"):
                    for ночь in (False, True):
                        ...
                        for х in м.values():
                            н = f"{метка} {х['ид']}"
                            if not х["вСтрокеКода"] or not х["кодПравее"]:
                                плохо(н + ": значок не слева от «Код» в строке кода")
                            if х["вне"]: плохо(н + ": значок за краем экрана")
                            if х["доКрая"] < 10: плохо(н + f": прилип к левому краю карточки ({х['доКрая']})")
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:40]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: у менеджера и администратора в строке кода слева от «Код» облако с галочкой — пресет в базе — или экран — ждёт связи; "
