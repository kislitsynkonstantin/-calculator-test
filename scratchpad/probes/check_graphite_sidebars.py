...
# Тоны-пары: фон меню, заголовок группы, выбранный пункт. «Бирюза-антрацит»
# (25.09.2026): антрацит и светлая бирюза #5fc2ca — 7:1, как зелёный на графите.
ПАРЫ = {"bmsk": (ГРАФИТ, ЗЕЛ, ЗЕЛ2),
        "anthracite": ("rgb(31, 42, 44)", "rgb(95, 194, 202)", "rgb(127, 208, 214)")}
ФОНЫ_ПАР = {п[0] for п in ПАРЫ.values()}
НАХОДКИ = []
...
def справка(бр, порт):
    ...
    for тон in ("bmsk", "anthracite", "teal", "sky"):
        ждём = тон in ПАРЫ
        ...
        где = f"справка · {тон}"
        if ждём and з["фон"] != ПАРЫ[тон][0]:
            плохо(где, f"меню {з['фон']}, ждали {ПАРЫ[тон][0]}")
        if not ждём and з["фон"] in ФОНЫ_ПАР:
            плохо(где, f"тёмный пары протёк в другой тон: меню {з['фон']}")
        if ждём and (з["группа"] != ПАРЫ[тон][1] or з["выбран"] != ПАРЫ[тон][2]):
            плохо(где, f"акценты: группа {з['группа']}, выбранный {з['выбран']}")
        if not ждём and any(п[1] in (з["группа"], з["выбран"]) for п in ПАРЫ.values()):
            плохо(где, "акценты пары протекли в другой тон")
        if ждём and з["признак"] != тон:
...
def база(бр):
    ...
    try:
        ...
        for имя in ("bmsk", "anthracite", "sky", None, "bmsk", "", "anthracite", "teal"):
            ждём = имя in ПАРЫ
            стр.evaluate("""(имя) => {
            ...
            if not ((фон == ПАРЫ[имя][0]) if ждём else (фон not in ФОНЫ_ПАР)):
                плохо(где, f"меню {фон}, тёмный пары ждали: {ждём}")
            if гр and ((гр == ПАРЫ[имя][1]) if ждём else any(гр == п[1] for п in ПАРЫ.values())) != ждём:
                плохо(где, f"заголовок блока {гр}, акцент пары ждали: {ждём}")
