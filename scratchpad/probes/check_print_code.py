...
Проба держит:
  • память браузера полна — пресет всё равно получает код, код уходит в
    журнал, а строка журнала говорит, что копия в браузере не записалась;
...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            if "не записалась" not in р["журнал"]:
                плохо("полная память: журнал не говорит, что копия в браузере не записалась")
