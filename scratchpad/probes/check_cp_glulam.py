...
Проба держит:
  ...
  • у бруса со ставками видны этажность, террасы и две поправки, подсказка
    говорит «рассчитывается автоматически»;
  • три цены сходятся с правилом на границах строк (10, 10,5, 30, 30,5 м²),
    с террасами, с остеклением, без отделки и с обеими поправками сразу;
  • проценты в подписях поправок берутся из базы;
  • площади фасада и кровли, вписанные над списком проектов, форма берёт сама;
  • без тёплого контура галочки ложатся на цену из «Взять из выбранного» или
    вписанную руками, а не обнуляют её, и снятая галочка возвращает цену до
    рубля;
...
def ожидание(этажность, тёплый, открытая=0, крытая=0, остекление=False, без=False):
    ...
    for с in строки:
        if с["max"] is None or тёплый <= с["max"]:
            ставка = с["rate"]
            break
    поправка = 1 + (КОЭФ["glazing"] if остекление else 0) - (КОЭФ["no_finish"] if без else 0)
...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            подписи = спросить(стр, "() => [cpGlazingPct.textContent, cpNoFinishPct.textContent]") or []
            if подписи != ["(+4%)", "(−8%)"]:
                плохо(f"проценты в подписях поправок не из базы: {подписи}")
            ...
            for этаж, тёплый, откр, крыт, ост, без in случаи:
                ...
                надо = ожидание(этаж, тёплый, откр, крыт, ост, без)
                if есть != надо:
            ...
            ГАЛКИ = "([о, б]) => { cpGlazing.checked = о; cpNoFinish.checked = б; cpRecalcPrices(); }"
            спросить(стр, "() => { ['cpWarm','cpOpen','cpClosed','cpPrice100','cpPrice150','cpPrice200'].forEach(ид => document.getElementById(ид).value = ''); cpGlazing.checked = false; cpNoFinish.checked = false; cpCopyFromSelected(); }")
            ...
            for о, б, множ in ((True, False, 1.04), (True, True, 0.96), (False, True, 0.92), (False, False, 1)):
                спросить(стр, ГАЛКИ, [о, б])
                надо = [int(ц * множ + 0.5) for ц in прайс]
                есть = спросить(стр, ЦЕНЫ)
                if есть != надо:
                    плохо(f"без контура, остекление {о}, без отделки {б}: цены {есть}, ждали {надо} — "
                          "поправки должны ложиться на цену прайса, а не обнулять её")
            спросить(стр, "() => { cpPrice150.value = '1 000 000'; cpЦенаВписана(1); }")
            спросить(стр, ГАЛКИ, [True, False])
            if (спросить(стр, ЦЕНЫ) or [0, 0])[1] != 1040000:
                плохо(f"вписанная руками цена 1 000 000 с остеклением стала {(спросить(стр, ЦЕНЫ) or [0, 0])[1]}, ждали 1 040 000")
            спросить(стр, ГАЛКИ, [False, False])
