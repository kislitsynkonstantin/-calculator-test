...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            # Без связи менеджер видит то же, что со связью: где лежит пресет,
            # ему без разницы (Константин, 26.09.2026).
            if р["тост"].strip() != "Пресет сохранён":
                плохо(f"без связи сообщение не то же, что со связью: {р['тост']!r}")
            if р["вБазе"]:
            ...
            # Выгрузка молчит: менеджеру не говорят, что пресет ждал связи
            # (Константин, 26.09.2026).
            if р["тост"].strip():
                плохо(f"выгрузка очереди заговорила: {р['тост']!r}")
