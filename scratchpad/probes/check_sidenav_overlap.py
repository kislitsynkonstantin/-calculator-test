...
Лишний сдвиг — такая же находка, как перекрытие. 22.09.2026, двумя снимками:
«когда открываешь боковую панель, окно сдвигается. Оно не должно сдвигаться,
панель его не перекрывает». Первое правило отдавало панели всё её место разом,
и окно уезжало влево на полтораста пикселей там, где заходило под панель на
полсотни. Поэтому проба меряет окно дважды — само по себе и при открытой
панели — и сверяет сдвиг с тем, который был нужен.

Проба меряет то, что видно, в обеих темах и на четырёх ширинах:

  • окно кончается левее панели, и между ними есть зазор;
  • сдвинуто оно ровно на столько, на сколько заходило под панель, — ни
    пикселем дальше; где заходить было нечем, окно стоит на месте;
...
ЦЕЛЬ = 32          # зазор, который закладывает само правило в index.html
...
ВИД = """([лист, подложка]) => {
  ...
  return {
    ...
    окноЛево: к ? Math.round(к.left) : null,
        ...
        with sync_playwright() as pw:
            ...
            for имя, вызов, лист, подложка, что in ОКНА:
                ...
                for тема in ("Модерн", "Бланк"):
                    for ширина in (1280, 1440, 1600, 1800):
                        ...
                        # Как окно стоит само по себе — это мерка, с которой
                        # сравнивается сдвинутое.
                        своё = стр.evaluate(ВИД, [лист, подложка])
                        ...
                        if not в["окноВидно"]:
                            ...
                            сдвиг = своё["окноЛево"] - в["окноЛево"]
                            # Сколько окну вообще нужно уйти, чтобы кончиться
                            # левее панели: разница между его собственным
                            # правым краем и местом под панелью.
                            надо = max(0, своё["окноПраво"] - (в["панельЛево"] - ЦЕЛЬ))
                            сузилось = своё["окноШирина"] - в["окноШирина"]
                            print(f"  {тема} {ширина}px: окно {в['окноШирина']} px, "
                                  f"правый край {в['окноПраво']}, панель с "
                                  f"{в['панельЛево']} · зазор {зазор} · сдвиг "
                                  f"{сдвиг} при нужном {надо}"
                                  + (f", сузилось на {сузилось}" if сузилось else ""))
                            if зазор < ЗАЗОР:
                                плохо(f"[{где}] окно заходит под боковую панель: "
                                      f"зазор {зазор} px — под ней {что}")
                            # Лишний сдвиг виден человеку как пустое поле между
                            # окном и панелью: 22.09.2026 окно уезжало влево на
                            # 150 px там, где под панель заходило на 50.
                            # Когда места рядом с панелью уже нет, окно сужается
                            # и упирается в левое поле — тогда меряем только
                            # зазор, сдвиг там задан краем листа, а не расчётом.
                            if not сузилось and сдвиг > надо + 6:
                                плохо(f"[{где}] окно сдвинуто дальше, чем нужно: "
                                      f"ушло на {сдвиг} px при нужных {надо} — "
                                      f"между ним и панелью пустое поле {зазор} px")
                            if not сузилось and сдвиг < надо - 6:
                                плохо(f"[{где}] окно сдвинуто меньше нужного: "
                                      f"{сдвиг} px против {надо}")
