...
def главная():
    ...
    with sync_playwright() as pw:
        ...
        # Тронули снимок — сдвига уже не будет, и надпись обязана вернуться.
        стр.evaluate("""() => {
        ...
        стр.wait_for_timeout(80)
        назад = надпись()
        if назад["текст"] != "Авто-расстановка" or назад["залито"]:
            НАХОДКИ.append(f"снимок подвинули, а кнопка всё ещё {назад['текст']!r} — "
