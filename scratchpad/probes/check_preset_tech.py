...
Плюс фильтр с мультивыбором и «Все», плюс нажатие по метке на карточке.
...
Проба меряет то, что видно, и на обеих ширинах:
  ...
  • чипы фильтра выбираются вместе, «Все» снимает выбор, счёт у чипа не
    меняется от собственного нажатия;
...
def главная():
        ...
        with sync_playwright() as pw:
            ...
            for ширина in (1440, 390):
                ...
                if ширина == 1440:
                    ...
                    в = стр.evaluate(ВИД)
                    if len(в["карточки"]) != 3:
                        плохо("выбраны каркас и брус вместе, а в списке "
                              f"{len(в['карточки'])} из трёх — мультивыбор не работает")
    ...
    print("Чисто: метка называет технологию снимка, стоит слева от суммы на "
          ...
          "отбирает нажатие у соседей; фильтр выбирается вместе, «Все» "
