...
26.09.2026 широкий экран переделан: «на широком экране название технологии
поставь вот сюда» — метка в «Моих» поднялась в строку названия, к правому
краю, над суммой.
...
Проба меряет то, что видно, и на обеих ширинах:
  ...
  • на 1440 метка в строке названия: середина на середине первой строки
    имени, правый край вровень с суммой, до имени не меньше 8 px, поле
    нажатия не ложится на сумму; на 390 — справа от суммы, на одной строке;
  • на 1440 дата сохранения стоит сразу за толщиной, в 4–16 px от неё, а не у
    суммы («возле цены она не нужна на широком экране», 26.09.2026);
  ...
  return {
    ...
    карточки: карточки.map(к => {
      ...
      return {
        ...
        середина: (мк.top + мк.bottom) / 2,
        серединаИмени: (() => { if (!имя) return null; const р = document.createRange(); р.selectNodeContents(имя);
          const п = р.getClientRects()[0]; return п ? (п.top + п.bottom) / 2 : null; })(),
        доИмени: имя ? мк.left - имя.getBoundingClientRect().right : 99,
        правыеКрая: Math.abs(мк.right - цк.right),
        верхСуммы: цк.top,
        верхПоля: Math.max(мк.top + вб, карт.top),  // вб отрицательный: поле выше метки
        низПоляНаСебе: мк.bottom - (parseFloat(с.bottom) || 0),
        дата: (() => { const пр = к.querySelector('.pcard-project'); const д = пр && пр.querySelector(':scope > span');
          if (!д || !пр.firstChild || пр.firstChild.nodeType !== 3) return null;
          const р = document.createRange(); р.selectNodeContents(пр.firstChild); const т = р.getBoundingClientRect(), дд = д.getBoundingClientRect();
          return { зазор: дд.left - т.right, однаСтрока: Math.abs((т.top + т.bottom) / 2 - (дд.top + дд.bottom) / 2) < 4 }; })(),
...
def проверить(стр, ширина):
    ...
    if широкий:
        for к in карточки:
            if к["серединаИмени"] is None or abs(к["середина"] - к["серединаИмени"]) > 1.5:
                плохо(f"[{мк}] «{к['имя']}»: метка не в строке названия "
                      f"(середина {round(к['середина'], 1)}, у имени {к['серединаИмени']})")
            if к["правыеКрая"] > 1:
                плохо(f"[{мк}] «{к['имя']}»: правый край метки не вровень с суммой "
                      f"({round(к['правыеКрая'], 1)} px)")
            if к["доИмени"] < 8:
                плохо(f"[{мк}] «{к['имя']}»: имя подходит к метке на {round(к['доИмени'])} px")
            д = к.get("дата")
            if not д or not д["однаСтрока"] or not (4 <= д["зазор"] <= 16):
                плохо(f"[{мк}] «{к['имя']}»: дата сохранения не сразу за толщиной ({д}) — "
                      "на широком экране она стоит за проектом, а не у суммы")
            if к["низПоляНаСебе"] > к["верхСуммы"]:
                плохо(f"[{мк}] «{к['имя']}»: поле нажатия метки ложится на сумму "
                      f"({round(к['низПоляНаСебе'] - к['верхСуммы'])} px)")
    else:
    ...
    for к in карточки:
        # Поле, обрезанное краем карточки, в счёт не идёт.
        высота = round(к["низПоляНаСебе"] - к["верхПоля"]) if широкий else к["поле"]["в"]
        порог = 36 if широкий else 44
        if высота < порог:
            плохо(f"[{мк}] поле нажатия метки {высота} px по высоте — меньше {порог}")
        if к["задеваетКнопку"]:
...
def главная():
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ:
            print("  ✗", н)
        sys.exit(1)
    print("Чисто: метка называет технологию снимка, на широком экране стоит "
          "в строке названия над суммой, на телефоне справа от суммы, нажимается и не "
