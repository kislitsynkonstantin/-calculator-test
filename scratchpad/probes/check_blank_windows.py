...
РЯД = """() => {
  ...
  for (let и = 0; и < кнопки.length - 1; и++) {
    ...
    const между = Math.round(б.лево - а.право);
    if (между < 2) шов.push((кнопки[и].id || 'кнопка') + ' и ' + (кнопки[и + 1].id || 'соседка')
      + ': поля нажатия сходятся (' + между + ' px) — нажатие у шва уходит соседу');
    const рамки = Math.round(кнопки[и + 1].getBoundingClientRect().left
                             - кнопки[и].getBoundingClientRect().right);
    if (рамки < 6) шов.push((кнопки[и].id || 'кнопка') + ' и ' + (кнопки[и + 1].id || 'соседка')
      + ': рамки стоят впритык (' + рамки + ' px)');
  }
