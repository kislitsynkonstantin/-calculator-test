...
и ночью, на 390 / 768 / 1440 px:
  ...
  • в «Модерне» днём название калькулятора в шапке, названия и суммы
    разделов — графит #24271f (Константин 25.09.2026: «в Модерн теме сделай
    графит»); в «Бланке» днём — только название раздела («названия разделов
    в „Зелёной-графит“ в теме Бланк перекрась в графит»), и заголовки
    карточек, черта под ними зелёная; ночью — прежние;
  • в «Бирюзовом» оба блока остались прежними — заливка var(--br-28);
  • ничего не вылезает за окно, ошибок страницы нет;
  • с 25.09.2026 всё то же — для «Бирюзы-антрацита» (антрацит #1f2a2c днём,
    #161f21 ночью, цена со скидкой светлой бирюзой #7fd0d6).
...
НАХОДКИ = []
# Тоны-пары: у каждого свой тёмный (итог, заголовки) и светлый цвет цены со
# скидкой на нём. «Бирюза-антрацит» добавлен 25.09.2026 тем же устройством.
ПАРЫ = {
    "bmsk":       {"д": "#24271f", "н": "#1b1d17", "г": "rgb(36, 39, 31)", "гн": "rgb(27, 29, 23)",
                   "свет": "rgb(166, 212, 116)", "град": "rgb(46, 50, 40)"},
    "anthracite": {"д": "#1f2a2c", "н": "#161f21", "г": "rgb(31, 42, 44)", "гн": "rgb(22, 31, 33)",
                   "свет": "rgb(127, 208, 214)", "град": "rgb(43, 56, 59)"},
}
ВСЕ_ТЁМНЫЕ = {в for п in ПАРЫ.values() for в in (п["д"], п["н"], п["г"], п["гн"])}
...
def проверить(бр, порт, ш, в, бланк, ночь, тон, снимок=False):
    ...
    заг = стр.evaluate("() => ['.header h1', '.section-header > span:first-child', '.section-total', '.card-title', '#imageCanvasCard .card-title'].map(с => getComputedStyle(document.querySelector(с)).color)")
    Г = ПАРЫ[тон]["г"] if тон in ПАРЫ else "rgb(36, 39, 31)"
    # «Модерн» днём — шапка, название и сумма раздела; «Бланк» днём —
    # название раздела и заголовки карточек («Выбор проекта», «Реквизиты»,
    # «Визуализация и планировка»); ночью — ничего.
    ждём = [False] * 5
    if тон in ПАРЫ and not ночь:
        ждём = [False, True, False, True, True] if бланк else [True, True, True, False, False]
    черта = стр.evaluate("() => getComputedStyle(document.querySelector('.card-title')).borderBottomColor")
    if бланк and тон in ПАРЫ and черта == Г:
        плохо(где, "черта под заголовком карточки стала графитовой — должна остаться зелёной")
    if [ц == Г for ц in заг] != ждём:
        плохо(где, f"шапка, название и сумма раздела: {заг}, графит ждали {ждём}")
    ...
    граф = (ПАРЫ[тон]["н"] if ночь else ПАРЫ[тон]["д"]) if тон in ПАРЫ else None
    if тон in ПАРЫ:
        ...
        if з["карточка"] in ВСЕ_ТЁМНЫЕ or зп["полоска"] in ВСЕ_ТЁМНЫЕ:
...
def печать(бр, порт, тон):
    ...
    for стиль, з in ещё.items():
        ...
        графит = any(п["град"] in з["фон"] or п["г"] in з["фон"] for п in ПАРЫ.values())
        свой = тон in ПАРЫ and (ПАРЫ[тон]["град"] in з["фон"] or ПАРЫ[тон]["г"] in з["фон"])
        if (тон in ПАРЫ) != графит or (тон in ПАРЫ and (not свой or з["спец"] != ПАРЫ[тон]["свет"])):
    ...
    for ночь in (False, True):
        ...
        ждём = (ПАРЫ[тон]["гн"] if ночь else ПАРЫ[тон]["г"]) if тон in ПАРЫ else None
        if (ждём and ф != ждём) or (not ждём and ф in ВСЕ_ТЁМНЫЕ) or not ф:
    ...
    if тон in ПАРЫ:
        if п["фон"] != ПАРЫ[тон]["д"]:
            плохо(где, f"блок итога {п['фон']}, ждали {ПАРЫ[тон]['д']}")
        for н in п["надписи"]:
            if н["к"] < 4.5:
                плохо(где, f"«{н['текст']}» {н['цвет']} — {н['к']}:1, нужно 4.5")
    elif п["фон"] in ВСЕ_ТЁМНЫЕ:
        плохо(где, "графит протёк в другой тон")
    if ошибки:
        плохо(где, "ошибки страницы: " + "; ".join(ошибки)[:200])
...
def главная():
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ:
            print("  ✗", н)
        sys.exit(1)
    print("Чисто: в «Зелёном-графите» и «Бирюзе-антраците» карточка и полоска итога — тёмный своей пары, "
          "каждая надпись на них читается, лист печати и лист клиента — тоже; в «Бирюзовом» всё как было.")
