...
и ночью, на 390 / 768 / 1440 px:
  ...
  • в «Модерне» днём название калькулятора в шапке, названия и суммы
    ...
    в „Зелёной-графит“ в теме Бланк перекрась в графит»); ночью — прежние;
...
def проверить(бр, порт, ш, в, бланк, ночь, тон, снимок=False):
    ...
    заг = стр.evaluate("() => ['.header h1', '.section-header > span:first-child', '.section-total'].map(с => getComputedStyle(document.querySelector(с)).color)")
    Г = "rgb(36, 39, 31)"
    # «Модерн» днём — шапка, название и сумма раздела; «Бланк» днём — только
    # название раздела; ночью — ничего.
    ждём = [False, False, False]
    if тон == "bmsk" and not ночь:
        ждём = [False, True, False] if бланк else [True, True, True]
