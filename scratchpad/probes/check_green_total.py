...
и ночью, на 390 / 768 / 1440 px:
  ...
  • в «Модерне» днём название калькулятора в шапке, названия и суммы
    ...
    графит»); ночью и в «Бланке» — прежние;
...
def проверить(бр, порт, ш, в, бланк, ночь, тон, снимок=False):
    ...
    заг = стр.evaluate("() => ['.header h1', '.section-header', '.section-total'].map(с => getComputedStyle(document.querySelector(с)).color)")
    графит_заг = all(ц == "rgb(36, 39, 31)" for ц in заг)
    if графит_заг != (тон == "bmsk" and not бланк and not ночь):
        плохо(где, f"шапка и заголовки разделов: {заг}")
