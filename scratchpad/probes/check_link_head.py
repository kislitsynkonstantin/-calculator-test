...
def проверить(страница, окно, ш, в):
    ...
    у = страница.evaluate("""() => ({
      ...
      строки: [...document.querySelectorAll('#bellMenu .nt-i .t')].map(э => э.textContent),
    ...
    for строка in у.get("строки") or []:
        ...
        if ИМЯ in строка and строка.index(ИМЯ) < строка.index(ПРОЕКТ):
