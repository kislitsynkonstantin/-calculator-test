        ...
        with sync_playwright() as pw:
            ...
            for ночь in (False, True):
                for ш, в in ((390, 844), (1440, 900)):
                    ...
                    # Графит и титан — не поворот, а своя насыщенность (третий аргумент).
                    for имя, сдв, нас in (("графит", 19, 0.3), ("титан", -150, 0.35)):
                        стр.evaluate(f"() => перекраситьПоТону(document, {сдв}, {нас})")
                        стр.wait_for_timeout(150)
                        т_ = стр.evaluate(СЧЁТ)
                        if т_["бир"]:
                            НАХОДКИ.append(f"[{где}] {имя}: осталось {т_['бир']} бирюзовых цветов: {т_['примеры']}")
                    стр.evaluate("() => перекраситьПоТону(document, 0, 1)")
                    стр.wait_for_timeout(200)
