...
Проба держит на 390 и 1440, в «Бланке» и «Модерне», днём и ночью:
  • у каждой карточки кнопка замка стоит сразу за «Открыть», перед скачиванием
    (Константин, 26.09.2026: «поменяй местами замок и скачать»);
...
СЦЕНАРИЙ = """async () => {
  ...
  const карта = () => Object.fromEntries([...document.querySelectorAll('#sharedPresetList .shared-pcard')].map(к => {
    ...
    return [к.dataset.scode, { есть: !!кн, выкл: кн ? кн.disabled : null, закрыт: кн ? кн.classList.contains('on') : null,
      послеОткрыть: !!(пред && пред.classList.contains('btn-shared-use')),
      передСкачиванием: !!(след && /downloadSharedPreset/.test(след.getAttribute('onclick') || '')),
...
def главная():
                ...
                for тема in ("blank", "light"):
                    for ночь in (False, True):
                        ...
                        for код, х in до.items():
                            ...
                            if not х["послеОткрыть"]: плохо(н + f" {код}: замок не сразу за «Открыть»")
                            if not х["передСкачиванием"]: плохо(н + f" {код}: за замком не скачивание")
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ[:40]:
            print("  ✗", н)
        raise SystemExit(1)
    print("Чисто: на «Общих» замок стоит за «Открыть» перед скачиванием, свой переключается, чужой выключен и показывает состояние, "
