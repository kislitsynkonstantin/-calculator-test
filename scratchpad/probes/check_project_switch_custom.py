...
Берега:
  ...
  • сообщение называет снятое, а запись «Проект изменён» в журнале несёт
    список снятых;
  • сумма, вписанная в опцию каталога «по запросу», при смене снимается,
    а с ней и отметка, если у нового проекта цены на опцию тоже нет
    (Константин 24.09.2026: «Сбрасывать»); сообщение и журнал её называют;
  • выбор того же проекта ничего не снимает — ни своих опций, ни сумм;
...
ШАГИ = """async () => {
  ...
  // опция «по запросу» с вписанной суммой — без цены и в А, и в Б
  const пз = OPTIONS.find(o => !o.included && document.getElementById('por_' + o.id)
    && !(PRICE_MATRIX[Б] || {})[o.id] && !o.price);
  вышло.пзЕсть = !!пз;
  if (пз) {
    const поле = document.getElementById('por_' + пз.id);
    поле.value = '55 000'; поле.dispatchEvent(new Event('input'));
    toggleOpt(пз.id); await пауза();
    вышло.пзИмя = пз.name;
    вышло.пзОтмеченаДо = !!checkedOptions[пз.id];
  }
  // своя опция
  ...
  await выбрать(А);
  вышло.темЖе = своих();
  вышло.суммаТемЖе = пз ? document.getElementById('por_' + пз.id).value : '';
  ...
  if (пз) {
    const поле = document.getElementById('por_' + пз.id);
    вышло.суммаПосле = поле ? поле.value : '(поля нет)';
    вышло.пзОтмеченаПосле = !!checkedOptions[пз.id];
  }
  ...
  const рядС = смена ? (смена.details.rows || []).find(р => р.k === 'Сняты вписанные суммы') : null;
  вышло.суммыВЖурнале = рядС ? рядС.b : null;
...
def главная():
    ...
    try:
        with sync_playwright() as pw:
            ...
            if в:
                ...
                if not в.get("пзЕсть"):
                    плохо("опции «по запросу» без цены в обоих проектах нет — сумму проверить нечем")
                else:
                    if not в["пзОтмеченаДо"]:
                        плохо("опция «по запросу» с вписанной суммой не отметилась — проверять нечего")
                    if not в["суммаТемЖе"]:
                        плохо("выбор того же проекта стёр вписанную сумму")
                    if в["суммаПосле"]:
                        плохо(f"после смены проекта вписанная сумма осталась: «{в['суммаПосле']}»")
                    if в["пзОтмеченаПосле"]:
                        плохо("опция без суммы осталась отмеченной — уйдёт в расчёт нулём")
                    if в["пзИмя"] not in (в["тост"] or ""):
                        плохо("сообщение не называет опцию со снятой суммой")
                    if not в["суммыВЖурнале"] or в["пзИмя"] not in в["суммыВЖурнале"]:
                        плохо(f"в записи «Проект изменён» нет снятых сумм: {в['суммыВЖурнале']!r}")
                if в["изПресета"] != 1:
    ...
    if НАХОДКИ:
        ...
        for н in НАХОДКИ:
            print("  ✗", н)
        sys.exit(1)
    print("Чисто: смена проекта снимает свои опции с их отметками и вписанные суммы "
          "с отметкой и говорит об этом, "
