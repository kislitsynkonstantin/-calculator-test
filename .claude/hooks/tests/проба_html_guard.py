...
# Новый выпуск начинает счёт заново — так в архиве: v2.5.6 (54) → v2.5.7 (25).
# Страж сравнивал одни номера и версию не смотрел, поэтому после выпуска не
# давал начать сначала, и v2.5.9 пошёл с (40) — продолжением чужого ряда.
ДРУГАЯ = ('<html><body><div id="loginVersionBadge" class="lg">v2.5.8 ({0})</div>'
          '<div class="version-badge" id="versionBadge">v2.5.8 ({1})</div>'
          '<script>var a = 1;</script></body></html>')
open(главный, 'w', encoding='utf-8').write(ДРУГАЯ.format(1, 1))
код, текст = позвать(главный)
факт(код == 0, f"новый выпуск начинает счёт заново: код {код}, {текст[:70]!r}")

# А внутри нового выпуска правило прежнее: назад нельзя.
subprocess.run(['git', '-C', репо, 'add', '-A'], check=True)
subprocess.run(['git', '-C', репо, 'commit', '-qm', 'выпуск'], check=True)
open(главный, 'w', encoding='utf-8').write(ДРУГАЯ.format(1, 1))
код, _ = позвать(главный)
факт(код == 0, "тот же номер внутри нового выпуска проходит")
open(главный, 'w', encoding='utf-8').write(ШАБЛОН.format(99, 99))
код, текст = позвать(главный)
факт(код == 0, f"возврат к прежней версии с большим номером проходит: {текст[:60]!r}")
