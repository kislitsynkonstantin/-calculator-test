...
def проверить_значок(путь, текст):
    ...
    if not прежние:
        return None
    прежняя_версия, прежний_номер = next(iter(прежние.values()))
    # Счётчик итераций считает подверсии внутри одного выпуска и начинается
    # заново с каждым новым: v2.5.6 (54) → v2.5.7 (25) → v2.5.8 (39) — так в
    # архиве, так и задумано. Страж сравнивал одни номера и версию не смотрел,
    # поэтому после выпуска не давал начать сначала: «счётчик пошёл назад».
    # 20.09.2026 v2.5.9 из-за этого начался с (40) — продолжением чужого ряда;
    # Константин заметил сразу: «после диплоя на боевой версию поменял, а
    # значения в скобках не обновил».
    if версия != прежняя_версия:
        return None
    if номер < прежний_номер:
        return (f"Счётчик итераций пошёл назад: в коммите ({прежний_номер}), "
                f"в файле ({номер}). Внутри выпуска номер не убывает даже при "
                f"откате — после ({прежний_номер}) идёт ({прежний_номер + 1}). "
                f"Начать заново можно, только сменив версию: у нового выпуска "
                f"свой счёт.")
    return None
